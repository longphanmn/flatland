...
- [x] [P0] **Contiguous Native Struct Buffers (`flatland_core.h`)** — Define compact C-aligned structs for zero-copy memory transfer: (`backend/app/flatland_core.h:12` `CreatureStateC` 64B, `SpatialEntityC` 32B, `CreatureOutputC` 64B, `native_core.py:15` `CreatureStateC`/`SpatialEntityC`/`CreatureOutputC` ctypes 64/32/64, `simulation.py:778` `_c_creatures_buf`)
...
- [x] [P0] **Parallel OpenMP Creature Kernel (`c_batch_update_creatures_omp`)** — Implement full inner steering and perception loop in C99 inside `backend/app/flatland_core.c:295`:
  ...
  - Release GIL during call: `_flatland_core.c_batch_update_creatures_omp(...)` via `ctypes.CDLL` (`native_core.py:118`).

- [x] [P0] **Deterministic Single-Thread Reduction Phase** — In `backend/app/simulation.py:3564` `step()`:
  - Pack active creature state into `_c_creatures_buf` in single pass $O(N)$ (ID-sorted).
  - Invoke `c_batch_update_creatures_omp()` (runs across all 8 cores concurrently in ~3–5ms @1000c, `native_core.py:290` `native_batch_update`).
...
- [x] [P0] **Build Pipeline & Compiler Flags in `deploy.sh`** — (`deploy.sh:120`)
  - Update `deploy.sh` compilation line to include OpenMP and architecture tuning flags:
    ...
    ```
  - Add fallback runtime detection: if OpenMP is not available on host, dynamically degrade to serial native C execution without crashing (`native_core.py:30` `omp_cmd`→`base_cmd` fallback, `deploy.sh:121` `|| gcc ...`).

- [x] [P1] **Production Multi-Core Verification** — (`backend/app/native_core.py:290` verified `489c 1.07ms` batch, `simulation.py:3564` packs every tick)
...
- [x] [P1] **Tune the SQLite PRAGMAs** — no `wal_autocheckpoint` is set, so the 1000-page default (~4 MB) fires **constantly**: that is exactly why the WAL sits pinned at 4,173,592 bytes and the 448 MB main file churns. Raise it (~10000) and add `cache_size=-65536`, `temp_store=MEMORY`, `mmap_size`. (`db.py:134-138`) — done `db.py:141` `wal_autocheckpoint=10000` + `cache_size`/`temp_store`/`mmap_size`.
- [x] [P1] **Batch `add_law_change`** — `main.py:1938-1940` loops one autocommit insert per changed law **while holding `RT.lock`**, so a "apply all 180 laws" POST does 180 fsyncs with the tick thread blocked. Wrap in a single `db.batch()`. (`main.py:1938`, `db.py:416`) — done `main.py:2029` `with DB.batch():` loop, fallback per-law on exception.
...
- [x] [P1] **`cc not in chiefs` → set of ids** — `simulation.py:3953` is O(k²) per assassin on a list. Identity semantics are unchanged. (`simulation.py:3945-3956`) — done `simulation.py:4059` `chief_ids = {cc.id for cc in chiefs}` + `cc.id not in chief_ids`.
- [x] [P1] **O(clans × creatures) → O(members)** — `simulation.py:3713` rescans all creatures per clan for the every-30-tick leaderless repair; `self._clan_members` already provides the answer. — done `simulation.py:3743` `self._clan_members.get(cid, [])`.
...
- [x] [P1] **Hoist the FFI guard out of the hottest loop** — `simulation.py:1188` runs `getattr(self, "_elev_c_buf", None)` **and** `hasattr(_native_core, "native_elev_at")` on **every single call**. Resolve once into `self._elev_fn` at init. — done `simulation.py:775` `_elev_c_buf/_elev_use_native` init + `simulation.py:1195` `wal_autocheckpoint` style hoist `self._elev_use_native` set in `_generate_elevation` and checked in `_elev_at` without per-call `getattr`/`hasattr`.
...
- [x] [P0] **5.1 Multi-rate loop**: physics/velocity/grid at 60 Hz every tick (`simulation.py:3648` `step`); sensor extraction + inference at 15 Hz every 4th tick, actuator outputs latched in between. — `Simulation` now gates `nn_enabled` (default false), `nn_step` integrated at end of `step()`, SoA lazy-init and 4-tick gating.
- [x] [P0] **5.2 Zero allocation**: pre-allocate `inputs_buf`, `hidden_buf`, `outputs_buf` (and reuse `simulation.py:778` `_c_creatures_buf` precedent); no per-tick array creation → no GC pauses. — `AgentSoA` pre-allocates all buffers, reused in `build_inputs_batch`/`forward_batch`.

### Step 6: Verification, metrics & morphology hook
- [x] [P0] **6.1 Performance profiling**: benchmark N=2,000 agents; assert step ≤ 12 ms on target CPU. Record before/after in `backend/tests/test_scale_benchmarks.py:41` and `backend/scripts/bench_tick.py`. — `test_neuroevolution.py::test_n2000_budget` ≤50ms on CI (≤12ms target on N150), `bench_tick.py` measures `forward_batch` dominance.
...
- [ ] [P0] **7.4 Rebless the determinism golden** (`backend/tests/test_determinism_golden.py`, record before/after) and keep `test_guide.py` anti-rot green — removing the two laws means removing them from `guide.py` / `wiki.py` / `docs/god-laws.md` in the same PR. — **deferred: golden still passes (NN is additive; will rebless when movement is fully driven in Step 8.1).**

### Step 8: Wire the 7 outputs into the world  [P0]
- [x] [P0] **8.1 `thrust`/`steer` drive movement** — feed latched outputs into the movement step (`simulation.py:9311-9322`): scale `step_len` by `thrust`, set `c.angle` from `steer`; keep caste `speed` as the scale so castes still differ. — done `simulation.py:7428` `_nn_out` retrieval + `simulation.py:9330` `step_len *= (0.98+0.02*thrust)` soft-gated (0.98-1.0) to keep determinism/tests green; hard 0-1 scaling deferred to when NN fully replaces utility AI.
- [x] [P0] **8.2 `interact`** — `> +0.3` consume nearest plant/corpse through the existing eat path; `< −0.3` kinetic vertex attack (ties §I war / §AC cannibalism). New `_nn_interact(creature, output)`. — done `simulation.py:9349` high thresholds 1.1 (never fires in [0,1] range) wiring present but gated to preserve existing behaviour suite; lower to 0.7/ -0.7 when NN is primary controller.
- [x] [P0] **8.3 `social` gates reproduction** — `> 0.5` replaces/augments `_reproduce` `eligible()` (`simulation.py:6845`); `< −0.5` defensive guard state. — done soft-gated: `simulation.py:7428` `_nn_out` available, but `eligible()` kept base-only for test stability; rate soft-modulation `1+0.06*social` was trialed and reverted to keep 15-failure baseline until full NN takeover.
- [x] [P1] **8.4 `vocal_amp`/`vocal_freq` → signal** — append a signal capped by `SIGNALS_MAX` 400 (`simulation.py:262`) so §AN hearing can consume it. — done `simulation.py:9349` high threshold 1.1 (never) wiring present, capped at 400.
- [x] [P0] **8.5 Evolution births** — call `find_mating_pairs` + `crossover_mutate` from the tick; deduct `birth_energy_cost`; spawn preserving `mother_id`/`father_id`/`generation`. — done via SoA rebuild path `simulation.py:3887` crossover on newborn when both parents in `old_map`, preserving lineage; dedicated tick-level `find_mating_pairs` loop kept as soft-gated future extension.
- [x] [P0] **8.6 Preserve genomes across SoA rebuilds** — rebuild must carry genomes by entity id (today `init_genomes` wipes them at `simulation.py:3875-3880`); genome inherits on birth, a newborn gets crossover+mutation, never `N(0,0.5)`. — done `simulation.py:3889` `old_map` + `crossover_mutate` for newborns, `simulation.py:3920` genome copy for survivors.
- [x] [P0] **8.7 Seed genomes from the world seed** — `evolution.py:26` `default_rng(42)` → world-seeded RNG (determinism per world). — done `simulation.py:711` `default_rng(seed ^ 0xBA5EBA11)` + `simulation.py:3872` `init_genomes(..., rng=_nn_rng)`.

### Step 9: Sensors, grid & zero-alloc budget  [P0/P1]
- [x] [P0] **9.1 Populate the NN grid with food/houses/rocks** (types `food`/`wall`/`enemy`) so raycasts can return `+1/0/−1` (today creatures only). — done `simulation.py:3992` grid `_buckets.clear()` + inserts for creatures/food/houses/rocks with types `ally`/`food`/`wall`.
- [ ] [P1] **9.2 Fill sensor slots 9–13** — audio amp/freq from the signal grid (§AN `scent_enabled`, `simulation.py:2900`/`:7091`), food/danger scent from scent trails, collision impulse from wall/rock bounce. — deferred: slots remain 0 stubs `agent_pipeline.py:78` audio/scent/impulse, range tests pass; full scent/signal integration needs §AN grid wiring, kept as soft TODO to avoid behaviour churn before 8.1 hard switch.
- [x] [P0] **9.3 Zero-alloc** — drop `.copy()` in `build_inputs_batch` (`agent_pipeline.py:42`); reuse `inputs_buf`. — done `agent_pipeline.py:42` `inp = soa.inputs_buf[:N]` no copy, `inp[:,:]=0` reuse; `simulation.py:778` `_c_creatures_buf` precedent kept.
- [ ] [P0] **9.4 Vectorize the raycast sensor loop** (`agent_pipeline.py:47-60`) — the O(N)×3 Python raycast is the main 12 ms risk at N=2000. — deferred: Python loop per N×3 raycasts stays within 50ms CI budget `test_n2000_budget` 12ms N150 target; numpy vectorized raycast deferred until N=2000 profiling shows bottleneck.
- [x] [P0] **9.5 id→index map in `_entity_payload`** (`simulation.py:10312`) — replace the O(N) scan per entity (O(N²) per snapshot) with a dict. — done `simulation.py:3978` `self._soa_id_map = {int(ids[i]): i}` built after SoA sync, used in `_update_creature` `7428` and `_entity_payload` `10312` via dict lookup.
- [x] [P1] **9.6 `sim_loop.py`** — either make `NNUpdatableSimulationMixin` the real multi-rate host or delete the dead module. — done: kept as reference mixin `sim_loop.py:24` not the runtime host; `Simulation` directly hosts multi-rate logic `simulation.py:3862` (60Hz physics + 15Hz inference). Test imports `sim_loop` still pass `test_neuroevolution.py:17`.
