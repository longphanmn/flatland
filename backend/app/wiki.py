...
- **Geometric Physics & Morphological Evolution (BC)**: Polar genomes $(r_i,\\phi_i)$ $K\\in[3,64]$ with SoA trait baking ($A,P,I_{zz},\\theta_{\\min}$) and SAT narrowphase (broadphase $r_{\\max}$ + edge normals); annealing $\\lambda(g)$ blends Abbott templates → free evolution, energetic asymmetry and neural courtship.
...
LAW_HINTS_MD = {
    ...
    "nn_inference_hz": "NN inference frequency per second (15) — every 4th tick latched, zero-alloc buffers — always on",
    "mutation_sigma": "Gaussian mutation σ for genome (0.08) — per-gene noise on crossover — always on",
    "crossover_rate": "crossover rate (0.5) — uniform 50/50 parent blend — always on",
    "morphology_annealing_enabled": "master switch for geometric physics — when off, creatures keep classic sides/irregularity path; when on, polar (r,φ) annealing governs shape, area/perimeter bake traits and SAT uses polygons (BC)",
    "annealing_start_generation": "generation where annealing decays from 1 → 0 (50) — before it, children snap to Abbott templates; after decay, free evolution",
    "annealing_decay_generations": "generations to decay λ 1→0 (150) — short = instant open-ended, long = classical persistence",
    "morph_lambda_override": "force λ 0..1 (None = auto annealing) — 1.0 freezes Abbott castes, 0.0 pure parental",
    "vertex_mutation_std": "Gaussian σ for radial distances r_i (0.05) — per-vertex noise clamped [0.2,2.5]",
    "angle_mutation_std": "Gaussian σ for polar angles φ_i (0.02) — sorted circularly to avoid bow-tie",
    "topological_mutation_rate": "chance to add/remove a vertex per birth, scaled by (1-λ) (0.01) — longest edge split / closest neighbor merge, K 3..64",
