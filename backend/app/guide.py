...
## Clans & social order (§C/I/P)
Founding generation seeds one clan per caste (`simulation.py:244`), children inherit mother's `clan_id` and procedural name (`CLAN_ADJECTIVES/NOUNS`) + totem `Wolf/Tree/Shield/Eye` if `totems_enabled` (`config.py:54`, `simulation.py:68` `TOTEMS`/`TOTEM_BUFF`, `CanvasRenderer.tsx:536` pole). Clan crest color on snapshot, totem pole beside house, `StateMessage.clans` carries `name`/`totem`/`leader_id`/`color`. Relations −100..+100 drift `relation_drift_rate` →0, threshold `alliance_threshold`/`rivalry_threshold` → `alliance`/`rivalry` events; shared feeding within `flock_radius` `+2`. Boids `cohesion_weight`/`alignment_weight`/`separation_weight` blended after food-seeking; social yielding `YIELD_RADIUS` 2.5. Territory (§P): each settlement anchors a `territory_radius` 14 circle (`config.py:49`, `simulation.py:793` `_update_territory`, `CanvasRenderer.tsx:514`) — members steer home when outside (`0.35×steer`), trespass inside a rival's circle sours relations `trespass_decay` (`protocol.py:127`) via `_bump_relation`. Totem buffs (§P): `Wolf` +2 `hunt_radius`/`1.10×speed`, `Tree` `1.25×` harvest, `Shield` `0.70×` war damage + `1.30×` regen, `Eye` `1.25×` perceive (`simulation.py:1310`/`simulation.py:1558`/`simulation.py:724`). Leadership (§P): founder is `leader_id` (`simulation.py:323`), death triggers succession to oldest living member → `succession` event (`protocol.py:129`, `simulation.py:1225`) if `succession_enabled` (`config.py:54`).
